...
(function($) {
  ...
  function prepareWinners(winners: Bracket, labels, teams, isSingleElimination: boolean, skipConsolationRound: boolean) {
    var rounds = Math.log(teams.length * 2) / Math.log(2);
    var matches = teams.length;
    var round

    for (var r = 0; r < rounds; r += 1) {
      if(labels[r]) round = winners.addRound(labels[r])
      else round = winners.addRound("WR"+(r+1))
  ...
  function prepareLosers(winners: Bracket, losers: Bracket, labels, teamCount: number) {
    var rounds = Math.log(teamCount * 2) / Math.log(2) - 1;
    var matches = teamCount / 2;

    for (var r = 0; r < rounds; r += 1) {
      for (var n = 0; n < 2; n += 1) {
	if(labels[n+2*r]) var round = losers.addRound(labels[n+2*r])
        else var round = losers.addRound('LR'+(n+2*r+1))
  ...
  function prepareFinals(finals: Bracket, winners: Bracket, losers: Bracket, labels,
                         skipSecondaryFinal: boolean, skipConsolationRound: boolean, topCon: JQuery) {
    if(labels[0]) var round = finals.addRound(labels[0])
    else var round = finals.addRound('FR1')
    var match = round.addMatch(function() {
      ...
      function(match) {
        ...
        if (!skipSecondaryFinal && (match.winner().name !== null && match.winner().name === losers.winner().name)) {
          ...
          var lab = labels[1] ? labels[1] : 'FR2';
	  var round = finals.addRound(lab, function() {
    ...
    losers.final().connectorCb(function(tC): Connector {
      ...
      height += (tC.height()+100) / 2
      return {height: -height, shift: -shift}
    })
  }

  function mkRound(bracket: Bracket, label: string,  previousRound: Round,
  ...
  function mkBracket(bracketCon: JQuery, results, mkMatch): Bracket {
    ...
    return {
      el: bracketCon,
      addRound: function(label: string, doRenderCb: () => boolean): Round {
        var id = rounds.length
        var previous = null
        if (id > 0)
          previous = rounds[id - 1]

        var round = mkRound(this, label, previous, id, !results ? null : results[id], doRenderCb, mkMatch)
        rounds.push(round)
        return round;
      },
      dropRound: function() {
  ...
  var JqueryBracket = function(opts: Options) {
    ...
    function mkMatch(round: Round, data: Array<TeamBlock>, idx: number,
      ...
      function teamElement(round: number, team: TeamBlock, isReady: boolean) {
        ...
        var tEl = $('<div class="team" style="height:24px"></div>');
        var nEl = $('<div class="label_br"></div>').appendTo(tEl)
          ...
          nEl.click(function() {
            ...
            function editor() {
              function done_fn(val, next: boolean) {
                ...
                renderAll(true)
                span.click(editor)
                var labels = opts.el.find('.team[data-teamid=' + (team.idx + 1) + '] div.label_br:first')
      ...
      var matchCon = $('<div class="match" style="height:48px"></div>')
      var teamCon = $('<div class="teamContainer style="height:24px"></div>')
      ...
      return {
        ...
        render: function() {
          ...
          teamCon.css('height', (this.el.height() / 2) + 'px');
    ...
    if (isSingleElimination) {
      ...
      wEl = $('<div class="bracket"></div>').appendTo(topCon)
      spa = $('<div class="spacer" style="height=100px"></div>').appendTo(topCon)
    ...
    prepareWinners(w, data.round_labels[0], data.teams, isSingleElimination, opts.skipConsolationRound)

    if (!isSingleElimination) {
      prepareLosers(w, l, data.round_labels[1], data.teams.length);
      prepareFinals(f, w, l, data.round_labels[2], opts.skipSecondaryFinal, opts.skipConsolationRound, topCon);
    }
