...
def GenerateD1List():
    ...
    query = """
        SELECT U.FFname from Teams as T
            ...
            ORDER BY T.wins DESC, T.pointsFor DESC
            LIMIT 6
        """
    cursor.execute(query, (year,))
...
def GenerateD3List():
    ...
    query = """
        SELECT U.FFname from Teams as T
            ...
            INNER JOIN Users as U ON (T.ownerID=U.FFid)
            WHERE T.year=%s AND L.tier=1
            ORDER BY T.wins DESC, T.pointsFor DESC LIMIT 2 OFFSET 12
        """
    cursor.execute(query, (year,))
    ...
    for league in [12087, 12088, 12089]:
        query = """
            SELECT U.FFname from Teams as T
                INNER JOIN Leagues as L ON (T.leagueID=L.id and T.year=L.year)
                INNER JOIN Users as U ON (T.ownerID=U.FFid)
                WHERE T.year=%s AND L.id=%s
                ORDER BY T.wins DESC, T.pointsFor DESC
                LIMIT 6 OFFSET 6
            """
        cursor.execute(query, (year, league))
...
def GenerateD4List():
    ...
    query = """
        SELECT U.FFname from Teams as T LEFT JOIN Teams_post P ON (T.teamID=P.teamID AND T.year=P.year) INNER JOIN Leagues as L ON (T.leagueID=L.id and T.year=L.year) 
            INNER JOIN Users as U ON (T.ownerID=U.FFid) 
            WHERE P.teamID IS NULL AND T.year=%s AND L.tier=3
        """
    cursor.execute(query, (year,))
...
def GenerateD2FillOrder():
    ...
    query = """
        SELECT U.FFname from Teams as T
            INNER JOIN Leagues as L ON (T.leagueID=L.id AND T.year=L.year)
            INNER JOIN Users as U ON (T.ownerID=U.FFid)
            WHERE T.year=%s AND (L.tier=1 OR L.tier=2 OR L.tier=3)
            ORDER BY T.pointsFor DESC
        """
    cursor.execute(query, (year,))
    ranks = cursor.fetchall()
